...
from modules.train_utils.spot_segmentation import train
...
def run(config_file=None, opts=None, save_special=False, save_dir_special= None):
    ...
    if cfg.DATASET.name == 'confocal' or cfg.DATASET.name == 'confocal_segment' or cfg.DATASET.name == 'neuronal':drop_last_val_test= True ## the last batch of confocal data haas only 1 image, it lead to an error
    ...
    rand_idx= np.random.randint(0, batch_size_train)
    try:
        plt.figure()
        plt.subplot(1,2,1)
        plt.imshow(x[rand_idx, 0])
        plt.title('image')
        plt.subplot(1,2,2)
        plt.imshow(y[rand_idx, 0])

...
from modules.train_utils_classification import train
...
from modules.models.classifiers import *
...
def run(config_file=None, opts=None, save_special=False, save_dir_special= None):
    ...
    decoder_upsample_net= upsampling_net_name(lambda_scale_factor= lambda_scale_factor, T= T, recon_img_size= img_size, init_method= decoder_upsample_init_method, Ht= modelH(m=1).detach(), custom_upsampling_bias= custom_upsampling_bias, upsample_postproc_block= lambda x:x)
    ...
    decoder= simple_mnist_classifier(img_size).to(device)
    
    #decoder_name(T, img_size, img_channels, channel_list, last_activation).to(device)

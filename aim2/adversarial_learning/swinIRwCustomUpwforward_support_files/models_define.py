...
from adversarial_learning.swinIRwCustomUpwforward_support_files.models import *
from adversarial_learning.swinIRwCustomUpwforward_support_files.losses import *
...
def define_G(opt, prev_cfg, modelA, modelH):
    opt_net = opt['netG']
    netG = SwinIR(upscale=opt_net['upscale'],
                    ...
                    resi_connection=opt_net['resi_connection'], prev_cfg= prev_cfg, modelA= modelA, modelH= modelH)
    if opt['is_train']:
        init_weights(netG,
...
class swinIR_generative_decoder(nn.Module):
    def __init__(self, opt_file_path= 'swinIR_support_files/opt.yaml', prev_cfg= None, modelA= None, modelH= None):
        ...
        self.netG = define_G(opt, prev_cfg, modelA, modelH)
        self.netG = self.model_to_device(self.netG)
        if self.is_train:
            self.netD = define_D(opt)
            self.netD = self.model_to_device(self.netD)
            if self.opt_train['E_decay'] > 0:
                self.netE = define_G(opt, prev_cfg, modelA, modelH).to(self.device).eval()
    ...
    def update_E(self, decay=0.999):
        ...
        for k in netG_params.keys():
            if k== 'modelH.weights':netE_params[k]= netG_params[k].data
            else:netE_params[k].data.mul_(decay).add_(netG_params[k].data, alpha=1-decay)
    ...
    def netG_forward(self):
        self.E = self.netG(self.X, self.m)
        self.Ht= self.netG.Ht
        self.yt_down= self.netG.yt_down 

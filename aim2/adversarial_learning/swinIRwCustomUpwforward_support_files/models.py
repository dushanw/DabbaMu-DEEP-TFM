...
from adversarial_learning.swinIRwCustomUpwforward_support_files.models_support_blocks import *
...
class SwinIR(nn.Module):
    ...
    def __init__(self, img_size=64, patch_size=1, in_chans=3,
        ...
        self.upsample_net= self.create_upsample_net(self.prev_cfg)
        ...
        elif self.upsampler == 'custom_v2':      # havent used: because custom_V2 intuitively should be used before SwinIR 
            T                              = self.prev_cfg.MODEL.MODEL_H.T
            lambda_scale_factor            = self.prev_cfg.MODEL.MODEL_A.lambda_scale_factor 
            img_size                       = self.prev_cfg.DATASET.img_size
            decoder_upsample_init_method   = self.prev_cfg.MODEL.MODEL_DECODER.upsample_net_init_method
            custom_upsampling_bias         = self.prev_cfg.MODEL.MODEL_DECODER.custom_upsampling_bias
            device                         = self.prev_cfg.GENERAL.device

            self.upsample_net= custom_v2(lambda_scale_factor= lambda_scale_factor, T= T, recon_img_size= img_size, init_method= decoder_upsample_init_method, Ht= None, custom_upsampling_bias= custom_upsampling_bias, upsample_postproc_block= lambda x:x)
    ...
    def forward(self, x, m):
        Ht= self.modelH(m)
        lambda_up, yt_down = self.modelA.compute_yt(x, Ht)

        self.Ht= Ht.detach()
        self.yt_down= yt_down.detach()
        
        x= self.upsample_net(yt_down)
        H, W = x.shape[2:]

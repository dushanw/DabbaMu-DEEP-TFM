...
from adversarial_learning.swinIRwforward_support_files.models import *
from adversarial_learning.swinIRwforward_support_files.losses import *

def define_G(opt, modelA, modelH, prev_cfg):
    opt_net = opt['netG']
    netG = SwinIR(upscale=opt_net['upscale'],
                    ...
                    resi_connection=opt_net['resi_connection'], modelA= modelA, modelH= modelH, prev_cfg= prev_cfg)
    if opt['is_train']:
        init_weights(netG,
...
class swinIR_generative_decoder(nn.Module):
    def __init__(self, opt_file_path= 'swinIR_support_files/opt.yaml', prev_cfg= None, modelA= None, modelH= None):
        ...
        self.netG = define_G(opt, modelA, modelH, self.prev_cfg)
        self.netG = self.model_to_device(self.netG)
        if self.is_train:
            self.netD = define_D(opt)
            self.netD = self.model_to_device(self.netD)
            if self.opt_train['E_decay'] > 0:
                self.netE = define_G(opt, modelA, modelH, self.prev_cfg).to(self.device).eval()
